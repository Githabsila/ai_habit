...
async def run_habit_checkpoint_17(bot):
    """17:00 — вечерняя контрольная точка по привычкам. Раньше после 12:00
    следующая проверка была только в 23:00 (risk-уведомление), и та молчит,
    если хотя бы одна привычка за день уже закрыта — то есть при частичном
    выполнении (1 из 4) пользователь мог не получить вообще ничего с
    полудня и до конца дня."""
    await _run_habit_checkpoint(bot, 17, "habit_checkpoint_17", "Вечернее напоминание привычек 17:00")


async def run_habit_checkpoint_22(bot):
    """22:00 — последняя контрольная точка по привычкам перед 23:00
    risk-уведомлением (то относится только к пользователям с активной
    серией и полностью пустым днём)."""
    await _run_habit_checkpoint(bot, 22, "habit_checkpoint_22", "Финальное напоминание привычек 22:00")
